...
const MyProfile = () => {
    ...
    const { data: recentPosts = [], isLoading: postsLoading } = useQuery({
        ...
        queryFn: async () => {
            const res = await axiosSecure.get(
                `/posts?email=${user?.email}&limit=3`
            );
            return res.data;
        },
        enabled: !!user?.email,
    ...
    if (userLoading || postsLoading) {
        return (
            <div className="p-4 bg-[#fb8b24]/10 min-h-screen">
                {/* Profile Skeleton */}
        ...
        <div className="p-4 bg-[#fb8b24]/10 min-h-screen">
            ...
            >
                <img
                    ...
                    className="w-24 h-24 rounded-full mx-auto mb-4 object-cover border-2 border-[#9a031e]"
                />
                <h2 className="text-xl font-bold text-center text-[#5f0f40]">
                    {userInfo.name || user?.displayName}
                </h2>
                <p className="text-[#0f4c5c] text-center">{userInfo.email || user?.email}</p>
                <p className="mt-2 text-center">
                    <span
                        className={`px-3 py-1 rounded-full text-white ${userInfo.isMember ? "bg-[#e36414]" : "bg-[#9a031e]"
                            }`}
                    >
            ...
            <motion.div
                ...
                className="p-6"
            >
                <h3 className="text-2xl text-[#5f0f40] font-bold mb-4 text-center">
                    Your Recent Posts
                </h3>
                {recentPosts.length === 0 ? (
                    <p className="text-[#0f4c5c] text-center">You have no posts yet.</p>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
